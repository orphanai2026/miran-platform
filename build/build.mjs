 ...
 * أداة دمج جاهزة.
 *
 * **تفسير "ملفًا واحدًا نهائيًا" المتَّبع هنا (يستحق مراجعة المالك):**
 * خريطة الصفحات الثماني (القسم 7) قرار مُقفَل ينص على ثماني صفحات منفصلة
 * بروابط مستقلة — هذا يتعارض حرفيًا مع "ملف واحد" للمنصة كاملة كوثيقة
 * واحدة. للتوفيق بين القرارين دون فتح أي منهما: هذا السكربت يُنتج **ملفًا
 * نهائيًا واحدًا مستقلًا لكل صفحة** من الصفحات الثماني (لا وحدات ES تحتاج
 * خادمًا، كل شيء مُضمَّن) — نفس روح "ملف واحد" التي كانت عليها مِران
 * الأصلية (كانت هي نفسها صفحة واحدة، ملف واحد). إن كان المقصود الفعلي ملفًا
 * واحدًا حرفيًا للمنصة الثمانية كاملة (توجيه داخلي بالـ views بدل روابط/
 * URLs منفصلة، كما كانت مِران الأصلية قبل التقسيم لثماني صفحات) فهذا يحتاج
 * قرارًا صريحًا جديدًا يُعاد فيه فتح خريطة الصفحات (القسم 7) — لم يحدث ذلك.
 *
